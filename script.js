...
function applyLanguage(lang) {
  ...
  if (!document.querySelector('[data-panel="book"]').hidden) renderBook()
}
...
function renderQr(text) {
  ...
  if (!value) {
    ...
    placeholder.style.color = 'rgba(55, 65, 81, 0.62)'
  ...
  if (!QRCodeCtor) {
    const placeholder = document.createElement('div')
    placeholder.className = 'mono'
    placeholder.style.color = 'rgba(127, 29, 29, 0.92)'
    placeholder.style.fontSize = '12px'
...
function generatePassword({ length, useUpper, useLower, useNumbers, useSymbols, excludeAmbiguous }) {
  ...
  const chars = []
  for (const pool of activePools) chars.push(pickChar(pool))

  const fullPool = activePools.join('')
  for (let i = chars.length; i < len; i += 1) chars.push(pickChar(fullPool))
...
async function prefillSiteFromActiveTab() {
...
function main() {
  ...
  prefillSiteFromActiveTab()
}

document.addEventListener('DOMContentLoaded', applyEnvironmentMode)
